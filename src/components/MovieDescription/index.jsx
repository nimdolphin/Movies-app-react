...
const MovieDescription = () => {
  ...
  const renderMovie = (
    movie?.map(({ name, ageLimit, annotation, bannerLink, posterLink, genres, trailerLink, rentalDateStart, rentalDateEnd }, id) => (
      <div key={id} className="mov">
        ...
        <div className="date-and-age">
          ...
          <div className="genres-container">
            <div className="genres">{`жанры `}</div>
            <div className="dotted-line"></div>

             <div className="genres"> 
              {genres.map(({name}, idx, genres) =>
                (<span key={idx} className="gentes-movie">
                    {genres.length - 1 === idx ? `${name}` : `${name}, `}
                  </span>)
              )}
            </div>
          </div>
